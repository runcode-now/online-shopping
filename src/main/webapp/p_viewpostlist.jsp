...
<html>
    ...
    <body style="font-family: Manrope">
        ...
        <section class="product spad" style="padding-top: 30px;">
            <div class="container">
                <form action="pub_viewpostlist" id="post_list_p" method="post">
                    <div class="ms-panel-header mb-3" style="border-bottom: 0;">
                        <h4 style="color: #FF8306;" class="bold">Danh sách bài viết</h4>
                    </div>

                    <div class="ms-panel-body">
                        <div class="row mx-3">
                            <div class="show mb-4 justify-content-between" style="font-family: sans-serif">
                                <div class="show">
                                    <p class="show__title">Hiển thị</p>
                                    <div class="show__select">
                                        <select name="quantityPost" id="quantityPost" class="form-control" cols="30" rows="10" onchange="movePage(1)">
                                            <option value="8" <c:if test="${quantity == 8}">selected</c:if>>8 bài viết</option>
                                            <option value="16" <c:if test="${quantity == 16}">selected</c:if>>16 bài viết</option>
                                            <option value="0" <c:if test="${quantity == 0}">selected</c:if>>Tất cả</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="common col-md-5">
                                        <p class="common__title">Tìm bài viết</p>
                                        <input type="text" class="form-control" name="textSearch" oninput="movePage(1)"
                                               id="textSearch" value="${param.textSearch}" style="min-height: 35px;"/>
                                </div>
                            </div>
                            ...
                            <div id="data" >
                                <div class="row property__gallery">
                                    <c:forEach items="${requestScope.lNews}" var="lp" varStatus="loop">
                                        <jsp:useBean class="com.team3.onlineshopping.dal.AccountDAO" id="acc_dao"/>
                                        <jsp:useBean class="com.team3.onlineshopping.dal.CustomerDAO" id="cus_dao"/>
                                        <div class="col-md-12 mb-4">
                                            <a class="d-flex gap-3" href="pub_postdetail?newsId=${lp.newsId}">
                                                <div class="product__item__pic">${lp.newsImgUrl}</div>
                                                <div class="product__item__text text-left" style="padding: 0; max-width: 75%">
                                                    <h6 class="mt-2" style="font-size:23px; font-family: sans-serif;">${lp.newsTitle}</h6>
                                                    <p>${acc_dao.getAccByEmpId(lp.emId).accName} • <span class="font-italic" style="font-size: 14px">${lp.newsCreatedDate}</span></p>
                                                </div>
                                            </a>
                                        </div>                             
                                    </c:forEach>
